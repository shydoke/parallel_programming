...
vector<vector<double>> multiply_matrices(
...
int main(int argc, char* argv[]) {
    ...
    auto read_time =
        duration_cast<microseconds>(end_read - start_read).count();

    if (n1 != n2) {
        cerr << "Error: matrix sizes don't match: "
             << n1 << " vs " << n2 << '\n';
    ...
    vector<vector<double>> result =
        multiply_matrices(mat1, mat2, n);

    auto end_mult = high_resolution_clock::now();

    auto mult_time =
        duration_cast<microseconds>(end_mult - start_mult).count();

    auto total_time =
        duration_cast<microseconds>(end_mult - start_total).count();
